...
class _ChristmasController(object):
    ...
    def mayRemoveChest(self):
        ...
        if giftBox is not None:
            return giftBox.getState() in (AnimatedGiftBox.BoxState.WAITING_FOR_OPEN_RESPONSE, AnimatedGiftBox.BoxState.DONE, AnimatedGiftBox.BoxState.OPENING)
        else:
            return False
    ...
    def getChristmasItemInfo(self, itemId, count = None, isNew = False):
        ...
        if item is not None:
            return ChristmasItemInfo(item, count, isNew)
        else:
            return
    ...
    def __getMyChests(self):
        ...
        if self.__clientChristmas:
            return self.__clientChristmas.getMyChests()
        return {}
