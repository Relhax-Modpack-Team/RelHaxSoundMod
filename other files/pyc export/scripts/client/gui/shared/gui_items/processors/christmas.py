...
class ToysConvertor(Processor):
    ...
    def _errorHandler(self, code, errStr = '', ctx = None):
        ...
        if len(errStr) and key % errStr in SYSTEM_MESSAGES.ALL_ENUM:
            return makeI18nError('christmasItem/conversion/errors/%s' % errStr)
        return makeI18nError('christmasItem/conversion/errors/server_error')
...
class ChestBonusReceiver(Processor):
    ...
    def _errorHandler(self, code, errStr = '', ctx = None):
        ...
        if len(errStr) and key % errStr in SYSTEM_MESSAGES.ALL_ENUM:
            return makeI18nError('christmasChest/open/errors/%s' % errStr)
        return makeI18nError('christmasChest/open/errors/server_error')
