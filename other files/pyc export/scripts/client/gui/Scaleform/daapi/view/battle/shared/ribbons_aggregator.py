...
class _Ribbon(object):
    ...
    def aggregate(self, ribbon):
        raise self.getType() == ribbon.getType() or AssertionError
...
class _IgnoreRuleByKill(_Rule):
    ...
    def isValid(self, ribbon):
        ...
        if etype in (BATTLE_EFFICIENCY_TYPES.BURN, BATTLE_EFFICIENCY_TYPES.RAM):
            return not self._vehIDs.issuperset(set(ribbon.getVehIDs()))
        return True
